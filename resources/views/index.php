<html>
    <head>
        <meta charset="UTF-8">
        <meta name="description" content="GSeven Webapp">
        <meta name="author" content="Henrik Lassu, Tamas Fogt">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimal-ui" />
        
        <link rel="stylesheet" href="lib/angular-material.css">
        <link rel="stylesheet" href="lib/angular-material-icons.css">
        <link rel="stylesheet" href="css/gApp.css">

        <base href="/">    
    </head>
    <body ng-app="gApp">
        <script src="lib/angular.js"></script>
        <script src="lib/angular-ui-router.js"></script>
        <script src="lib/angular-aria.js"></script>
        <script src="lib/angular-messages.js"></script>
        <script src="lib/angular-animate.js"></script>
        <script src="lib/angular-material.js"></script>
        <script src="lib/angular-material-icons.js"></script>
        <script src="js/gApp.js"></script>

        <div ui-view></div>
    </body>
</html>
